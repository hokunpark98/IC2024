...
D_URL = "http://kube-bench-server-service:15018"  # D 서비스의 주소와 포트
...
def send_file_to_C(filesize):
    ...
    with open(file_path, 'rb') as f:
        files = {'fileC': (file_path, f, 'application/octet-stream')}
        response = requests.post(f"{D_URL}/receive-file", files=files)
...
if __name__ == "__main__":
    app.run(host='0.0.0.0', port=15017)  # B 서비스의 포트