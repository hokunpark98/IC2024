...
class Replacement:
        ...
        for source_pod in traffic_map:
            for target_pod in traffic_map[source_pod]:
                ...
                if source_pod_node != base_node or target_pod_node != base_node:                  
                    #temp[source_pod][target_pod]: [전송 횟수. 전송량]
                    total_traffic = round(traffic_map[source_pod][target_pod][1], 2) #MB로
                    data_size = round(traffic_map[source_pod][target_pod][1] / traffic_map[source_pod][target_pod][0] / 1024 * 1024, 1) #MB로
                    
                    #5분 동안 데이터 전송량
                    #print('total_traffic', total_traffic)
                    score = total_traffic #* latency * data_size, 1)
                    
                    if score > self.threshold:
                        replacementpod_different_node[f'{source_pod}/{target_pod}'] = score 
    ...
    def make_base_node(self): # 네임스페이스 별로 베이스 노드 생성
        pod_counts_per_namespace = self.node_info.get_pod_counts_per_namespace(self.namespaces) # 네임스페이스 별 파드 개수
  
        base_nodes = {}
        node_usage_count = {node: 0 for node in self.node_info.get_all_worker_nodes()}
        print(pod_counts_per_namespace)
        for namespace in pod_counts_per_namespace:
            
            pod_count_by_namespace = self.node_info.get_pod_count_by_namespace(namespace) #네임스페이스가 같은 파드가 가장 많은 노드 순으로 출력
            print(namespace, pod_count_by_namespace)
            min_usage = min(node_usage_count.values())
            
            for node in pod_count_by_namespace: # namespace 많은 순
                if node_usage_count[node] <= min_usage:
                    base_nodes[namespace] = node
                    node_usage_count[node] = node_usage_count[node] + 1
                    break
    ...
    def make_base_node_before(self): # 네임스페이스 별로 베이스 노드 생성
        pod_counts_per_namespace = self.node_info.get_pod_counts_per_namespace(self.namespaces)
       
        base_nodes = {}
        used_nodes = set()

        for namespace in pod_counts_per_namespace:
            pod_count_by_namespace = self.node_info.get_pod_count_by_namespace(namespace)
            

            for node in pod_count_by_namespace:
                if node not in used_nodes:
                    base_nodes[namespace] = node
                    used_nodes.add(node)
                    break
            else:
                available_nodes = set(self.node_info.get_all_worker_nodes()) - used_nodes
                if available_nodes:
                    selected_node = available_nodes.pop()
                    base_nodes[namespace] = selected_node
                    used_nodes.add(selected_node)
                else:
                    for node in self.node_info.get_all_worker_nodes():
                        if node not in base_nodes.values():
                            base_nodes[namespace] = node
                            break
    ...
    def replacement(self):
        base_nodes = self.make_base_node()   
        print('baseNode', base_nodes)   
        ...
        for namespace in namespaces_sort:   
            ...
            base_node = base_nodes[namespace] # 해당 네임스페이스의 베이스 노드
            ...
            print('allocatable_base_node', self.allocatable_resources[base_node]['allocatable_cpu'])
